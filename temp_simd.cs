...
namespace SortingNetworks.Generators
{
    ...
    internal static class SimdX86Emitter
    {
        ...
        // --- Byte (8-bit) types: single Vector256 for sizes Γëñ 32 ---
        ...
        {
            // AVX-512 BW: single Vector512 for Γëñ 32 elements
            if (size > 32) return ("", "");
        ...
        {
            // AVX2: single Vector256<ushort> for Γëñ 16 elements
            if (size > 16) return ("", "");
            ...
            if (size <= 8)
            {
                // 8 elements = 16 bytes ΓåÆ lower Vector128
                sb.AppendLine("            var lo = System.Runtime.CompilerServices.Unsafe.ReadUnaligned<System.Runtime.Intrinsics.Vector128<byte>>(ref first);");
            ...
            {
                // 16 elements = 32 bytes ΓåÆ full Vector256
