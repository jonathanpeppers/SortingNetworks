...
- Use branchless `Math.Min`/`Math.Max` compare-and-swap for numeric primitive types (`byte`, `sbyte`, `short`, `ushort`, `int`, `uint`, `long`, `ulong`, `float`, `double`). The JIT lowers these to `cmov` on x64. For `char` and custom types, use branching `if (a > b) { T temp = a; a = b; b = temp; }`.
